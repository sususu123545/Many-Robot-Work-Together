...
def move():
    ...
    while __isRunning:
        if arm_move and detect_color != 'None': # 等待可以夹取
            ...
            set_rgb(range_rgb[target_color][2], range_rgb[target_color][1], range_rgb[target_color][0])    # 设置rgb灯颜色
            rospy.sleep(0.1)
            ...
            if target:
                servo_data = target[1]
                bus_servo_control.set_servos(joints_pub, 1.2, ((6, servo_data['servo6']),)) # 机械臂先转过去
                rospy.sleep(1)
                bus_servo_control.set_servos(joints_pub, 1.5, ((3, servo_data['servo3']), (4, servo_data['servo4']), (5, servo_data['servo5']))) # 再放下了
            rospy.sleep(1.8)
